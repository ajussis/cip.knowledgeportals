...
The ProType content type
===============================

In this section we are tesing the ProType content type by performing
basic operations like adding, updadating and deleting ProType content
items.

Adding a new ProType content item
--------------------------------
...
'ProType' and click the 'Add' button to get to the add form.

    >>> browser.getControl('ProType').click()
    >>> browser.getControl(name='form.button.Add').click()
    >>> 'ProType' in browser.contents
    True

Now we fill the form and submit it.

    >>> browser.getControl(name='title').value = 'ProType Sample'
    >>> browser.getControl('Save').click()
    >>> 'Changes saved' in browser.contents
    True

And we are done! We added a new 'ProType' content item to the portal.

Updating an existing ProType content item
---------------------------------------

Let's click on the 'edit' tab and update the object attribute values.

    >>> browser.getLink('Edit').click()
    >>> browser.getControl(name='title').value = 'New ProType Sample'
    >>> browser.getControl('Save').click()
...
We check that the changes were applied.
    ...
    >>> 'Changes saved' in browser.contents
    True
    >>> 'New ProType Sample' in browser.contents
    True

Removing a/an ProType content item
--------------------------------

If we go to the home page, we can see a tab with the 'New ProType
...
Now we are going to delete the 'New ProType Sample' object. First we
go to the contents tab and select the 'New ProType Sample' for
deletion.

    >>> browser.getLink('Contents').click()
    >>> browser.getControl('New ProType Sample').click()
...
So, if we go back to the home page, there is no longer a 'New ProType
Sample' tab.
...
Adding a new ProType content item as contributor
------------------------------------------------

Not only site managers are allowed to add ProType content items, but
...
We select 'ProType' and click the 'Add' button to get to the add form.
...
Done! We added a new ProType content item logged in as contributor.
...
The ProjectType content type
===============================

In this section we are tesing the ProjectType content type by performing
basic operations like adding, updadating and deleting ProjectType content
items.

Adding a new ProjectType content item
--------------------------------
...
Then we select the type of item we want to add. In this case we select
'ProjectType' and click the 'Add' button to get to the add form.

    >>> browser.getControl('ProjectType').click()
    >>> browser.getControl(name='form.button.Add').click()
    >>> 'ProjectType' in browser.contents
    True

Now we fill the form and submit it.

    >>> browser.getControl(name='title').value = 'ProjectType Sample'
    >>> browser.getControl('Save').click()
    >>> 'Changes saved' in browser.contents
    True

And we are done! We added a new 'ProjectType' content item to the portal.

Updating an existing ProjectType content item
---------------------------------------

Let's click on the 'edit' tab and update the object attribute values.

    >>> browser.getLink('Edit').click()
    >>> browser.getControl(name='title').value = 'New ProjectType Sample'
    >>> browser.getControl('Save').click()
...
We check that the changes were applied.
    ...
    >>> 'Changes saved' in browser.contents
    True
    >>> 'New ProjectType Sample' in browser.contents
    True

Removing a/an ProjectType content item
--------------------------------

If we go to the home page, we can see a tab with the 'New ProjectType
Sample' title in the global navigation tabs.
...
Now we are going to delete the 'New ProjectType Sample' object. First we
go to the contents tab and select the 'New ProjectType Sample' for
deletion.

    >>> browser.getLink('Contents').click()
    >>> browser.getControl('New ProjectType Sample').click()
...
So, if we go back to the home page, there is no longer a 'New ProjectType
Sample' tab.
...
Adding a new ProjectType content item as contributor
------------------------------------------------

Not only site managers are allowed to add ProjectType content items, but
also site contributors.
...
We select 'ProjectType' and click the 'Add' button to get to the add form.
...
Done! We added a new ProjectType content item logged in as contributor.
